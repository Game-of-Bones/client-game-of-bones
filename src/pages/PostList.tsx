...
const PostList = () => {
  ...
  useEffect(() => {
    fetchPosts(); // Llama al backend una sola vez al montar el componente
  }, [fetchPosts]);

  if (isLoading) return <p className="text-center">Cargando descubrimientos...</p>;
  if (error) return <p className="text-center text-red-500">{error}</p>;
  if (posts.length === 0) return <p className="text-center">No hay posts publicados todavía.</p>;
